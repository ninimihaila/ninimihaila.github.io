...
audioTag.addEventListener('ended', stopDrums);
...
drums.addEventListener('play', () => drums.playbackRate = drumsBPM/120)
...
Array.from(document.querySelectorAll('a')).map(a=>a.addEventListener('click', l=>queryInput.value = a.textContent))
...
document.querySelector('form').addEventListener('submit', function(formEvent) {
  ...
  spotifyApi.searchTracks(
    queryInput.value.trim(), {limit: 10})
    .then(function(results) {
      // console.log(results.tracks.items.map(i=>i.preview_url))
      var track = results.tracks.items.filter(i=>i.preview_url)[0];
      ...
      request.onload = function() {
        ...
        offlineContext.oncomplete = function(e) {
          ...
          // debugger
          if (isFirefox) {
            drums.currentTime = peaks[0].position / 22050;
          } else {
            drums.currentTime = drums.duration - peaks[0].position / 22050;
          }
          console.log(drums.currentTime)
          console.log(peaks[0].position)
          console.log((peaks[0].position/22050) + 's')
          drumsBPM = top[0].tempo;
          ...
          spotifyApi.getAudioFeaturesForTrack(track.id)
            .then(function(audioFeatures) {
              ...
              if (Math.abs(drumsBPM - audioFeatures.tempo) > 2) {
                drumsBPM = audioFeatures.tempo;
              }
            });
